		...
		try (DirectoryStream<Path> stream = Files.newDirectoryStream(p)) {
			...
			for (Path file : stream) {
				...
				if (n == fileName.length()) {
					...
					System.out.println(prefixDirectory + "\nindex:" + prefixDirectory.lastIndexOf(' ') + "\nsize:"
							+ prefixDirectory.length());
					...
					try (BufferedWriter writer = Files.newBufferedWriter(newSPath.resolve(FILE_NAME), charset)) {
			...
			Charset charset = Charset.forName("UTF-8");
			try (BufferedWriter writer = Files.newBufferedWriter(sPath.resolve(FILE_NAME), charset)) {
				String line = data.toString();
				writer.write(line);
			} catch (IOException x) {
				System.err.format("IOException: %s%n", x);
