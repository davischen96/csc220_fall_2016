...
public class Main {
	...
	public static double accurateTime(Fib fib, int n) {
		...
		int ncalls = (int) (1000000 / (time * time));
		...
		while (true) {
			...
			while (true) {
				try {
					...
					if (nTwo < 0)
						throw new NumberFormatException("n cannot be less than 0");

					// If this not the first experiment, estimate the running
					// time for
					// that n using the value of the constant from the last
					// time.
					if (constant > 0) {
						...
						// ADD LATER: If it is greater than an hour, ask the
						// user for
						// confirmation before running the experiment.
						// If not, the "continue;" statement will skip the
						// experiment.
						...
						// Now, calculate the (accurate) running time for that
						// n.
